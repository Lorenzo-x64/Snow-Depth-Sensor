...
void checkBMI160() {
  ...
  if (mag < BMI160_FALL_THRESHOLD_G) {
    ...
    if (inFreeFall && (now - freeFallStartMs >= BMI160_FALL_WINDOW_MS) && !cooldownActive) {
      fallCount++;
      lastAlertTime = now;
      Serial.printf("[ IMU  ] ⚠ FREE FALL detected! magnitude=%.3f g\n", mag);
      sendAlertLoRa("FALL");
      logAlertToSD("FALL");
      inFreeFall = false; // reset after alert
    }
  } else {
    inFreeFall = false;
  }
...
async function initOrUpdateMap() {
  // Re-fetch data to get latest GPS position
  let d = null;
  try {
    const r = await fetch('/data');
    d = await r.json();
  ...
  if (!mapInitialized) {
    ...
    mapInitialized = true;
    // Delay invalidateSize to let the panel render fully
