/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║   SNOW STATION — LoRa Transmitter v4  (WiFi Dashboard)           ║
 * ║   Board  : ESP32                                                 ║ 
 ...
 *
 *  v3 → v4 additions:
 *    • BMI160 6-DOF IMU via I2C (SDA=21, SCL=22) — fall & impact detection
 *    • SD card full warning via LoRa
 *    • Fixed web UI map (Leaflet invalidateSize timing)
 *    • Dashboard IMU card + Alerts card + sidebar alerts count
 *    • /data JSON now includes bmi160_ready, fall_count, impact_count,
 *      last_alert, sd_nearly_full, accel data, fall_log_count
 *
 *  Dashboard panels
 *    1. Overview       — snow depth hero, GPS, LoRa TX, SD status, IMU
 *    2. GPS Map        — Leaflet / OSM live marker (fixed timing)
 *    3. LoRa TX        — transmission stats, last payload, alerts card
 *    4. SD Logger      — file status, row count, CSV download
 *    5. Accelerometer  — BMI160 live X/Y/Z, magnitude, peak, event log
 *    6. System         — RAM, Flash, CPU, uptime, WiFi AP info
 *
 *  Libraries required:
 *    TinyGPSPlus, LoRa_E220, SoftwareSerial, WiFi, WebServer, BMI160Gen
 */
...
#define BMI160_SDA_PIN    21
#define BMI160_SCL_PIN    22
...
// ── BMI160 thresholds ─────────────────────────────
#define BMI160_FALL_THRESHOLD_G      0.3f   // magnitude < 0.3g → free fall
#define BMI160_FALL_WINDOW_MS        150    // consecutive low-g window (ms)
#define BMI160_IMPACT_THRESHOLD_G    3.5f   // magnitude > 3.5g → impact
#define BMI160_ALERT_COOLDOWN_MS     10000  // 10 seconds between alerts
#define SD_FULL_CHECK_INTERVAL_MS    300000 // 5 minutes between SD full checks
#define SD_FULL_THRESHOLD_PCT        90.0f

// ── Accelerometer live tracking & logging ──────────
#define ACCEL_LOG_FILE              "/accellog.csv"
#define ACCEL_LOG_THRESHOLD_G       2.0f   // log event to SD when magnitude > this
#define ACCEL_LOG_MAX_ENTRIES       30     // in-memory ring buffer for web UI

// ── Fall detection event log ──────────────────────
#define FALL_LOG_FILE              "/falllog.csv"
#define FALL_LOG_MAX_ENTRIES       20
...
uint32_t lastAlertTime   = 0;     // millis of last IMU alert (cooldown)
bool     inFreeFall      = false;
uint32_t freeFallStartMs = 0;
char     lastAlertType[20] = "---"; // "FALL", "IMPACT", or "SD_FULL"
char     lastAlertTs[22]   = "---"; // timestamp string

// ── Live accelerometer data ───────────────────────
float    liveAccelX  = 0;
float    liveAccelY  = 0;
float    liveAccelZ  = 0;
float    liveAccelMag = 0;
...
// ── Acceleration event log (ring buffer + SD) ─────
struct AccelEvent {
  float  mag, ax, ay, az;
  char   ts[22];
};
AccelEvent accelLog[ACCEL_LOG_MAX_ENTRIES];
int accelLogHead  = 0;
int accelLogCount = 0;
uint32_t lastAccelLogMs = 0;       // throttle SD writes for accel events
#define ACCEL_LOG_COOLDOWN_MS     500  // minimum 500 ms between logged events

// ── Fall event log (ring buffer + SD) ─────────────
struct FallEvent {
  float  mag, ax, ay, az;
  uint32_t durationMs;
  char   ts[22];
};
FallEvent fallLog[FALL_LOG_MAX_ENTRIES];
int fallLogHead  = 0;
int fallLogCount = 0;

// ── SD full detection state ──────────────────────
bool     sdNearlyFull       = false;
bool     sdFullAlertSent    = false;
uint32_t lastSdFullCheckMs  = 0;
...
// helper: build "YYYY-MM-DD,HH:MM:SS" timestamp string
void buildTimestamp(char *buf, size_t len) {
  int h, m, s, d, mo, y;
  localTime(h, m, s, d, mo, y);
  snprintf(buf, len, "%04d-%02d-%02d,%02d:%02d:%02d", y, mo, d, h, m, s);
}

// ═══════════════════════════════════════════════════
//  BMI160 IMU — Fall & Impact Detection
// ═══════════════════════════════════════════════════

void sendAlertLoRa(const char *type) {
  if (!loraReady) return;
  ...
  char msg[64];
  snprintf(msg, sizeof(msg), "ALERT:%s,%s", type, ts);

  lora.sendMessage(msg);
  loraCount++;

  strncpy(lastAlertType, type, sizeof(lastAlertType) - 1);
  strncpy(lastAlertTs, ts, sizeof(lastAlertTs) - 1);

  Serial.printf("[ IMU  ] LoRa alert sent: %s\n", msg);
}

void logAlertToSD(const char *type) {
  if (!sdReady) return;
...
void logAccelEvent(float mag, float ax, float ay, float az) {
  // Store in in-memory ring buffer
  char ts[24];
  buildTimestamp(ts, sizeof(ts));

  AccelEvent& ev = accelLog[accelLogHead];
  ev.mag = mag; ev.ax = ax; ev.ay = ay; ev.az = az;
  strncpy(ev.ts, ts, sizeof(ev.ts) - 1);
  ev.ts[sizeof(ev.ts) - 1] = '\0';
  accelLogHead = (accelLogHead + 1) % ACCEL_LOG_MAX_ENTRIES;
  if (accelLogCount < ACCEL_LOG_MAX_ENTRIES) accelLogCount++;

  // Log to SD card
  if (!sdReady) return;
  File f = SD.open(ACCEL_LOG_FILE, FILE_APPEND);
  if (!f) return;
  f.printf("%s,%.3f,%.3f,%.3f,%.3f\n", ts, mag, ax, ay, az);
  f.close();
}

void logFallEvent(float mag, float ax, float ay, float az, uint32_t duration) {
  char ts[24];
  buildTimestamp(ts, sizeof(ts));

  FallEvent& ev = fallLog[fallLogHead];
  ev.mag = mag; ev.ax = ax; ev.ay = ay; ev.az = az;
  ev.durationMs = duration;
  strncpy(ev.ts, ts, sizeof(ev.ts) - 1);
  ev.ts[sizeof(ev.ts) - 1] = '\0';
  fallLogHead = (fallLogHead + 1) % FALL_LOG_MAX_ENTRIES;
  if (fallLogCount < FALL_LOG_MAX_ENTRIES) fallLogCount++;

  // Log to SD card
  if (!sdReady) return;
  File f = SD.open(FALL_LOG_FILE, FILE_APPEND);
  if (!f) return;
  f.printf("%s,%.3f,%u,%.3f,%.3f,%.3f\n", ts, mag, duration, ax, ay, az);
  f.close();
  Serial.printf("[ FALL ] Logged to SD: %.3f g, duration %u ms\n", mag, duration);
}

void checkBMI160() {
  if (!bmi160Ready) return;
  ...
  if (mag > peakMag) {
    ...
    char ts[24];
    buildTimestamp(ts, sizeof(ts));
    strncpy(peakTs, ts, sizeof(peakTs) - 1);
    peakTs[sizeof(peakTs) - 1] = '\0';
  }

  uint32_t now = millis();

  // Cooldown check
  bool cooldownActive = (now - lastAlertTime) < BMI160_ALERT_COOLDOWN_MS;

  // ── Log big acceleration to SD (separate from alerts) ──
  if (mag > ACCEL_LOG_THRESHOLD_G && (now - lastAccelLogMs >= ACCEL_LOG_COOLDOWN_MS)) {
    lastAccelLogMs = now;
    logAccelEvent(mag, ax, ay, az);
    Serial.printf("[ ACCEL] Logged event: %.3f g (X=%.3f Y=%.3f Z=%.3f)\n", mag, ax, ay, az);
  }

  // ── Free-fall detection ──
  if (mag < BMI160_FALL_THRESHOLD_G) {
    if (!inFreeFall) {
      inFreeFall = true;
      freeFallStartMs = now;
    }
    // If free-fall sustained for the window and cooldown expired → alert
    if (inFreeFall && (now - freeFallStartMs >= BMI160_FALL_WINDOW_MS) && !cooldownActive) {
      uint32_t fallDur = now - freeFallStartMs;
      fallCount++;
      lastAlertTime = now;
      Serial.printf("[ IMU  ] ⚠ FREE FALL detected! magnitude=%.3f g (duration: %lu ms)\n", mag, fallDur);
      sendAlertLoRa("FALL");
      logAlertToSD("FALL");
      logFallEvent(mag, ax, ay, az, fallDur);
      inFreeFall = false;
    }
  } else {
    inFreeFall = false;
  ...
  // ── Impact detection ──
  if (mag > BMI160_IMPACT_THRESHOLD_G && !cooldownActive) {
    impactCount++;
    lastAlertTime = now;
    Serial.printf("[ IMU  ] 💥 IMPACT detected! magnitude=%.2f g\n", mag);
    sendAlertLoRa("IMPACT");
    logAlertToSD("IMPACT");
  }
}
...
void checkSdFull() {
  if (!sdReady) return;
  uint32_t now = millis();
  if (now - lastSdFullCheckMs < SD_FULL_CHECK_INTERVAL_MS) return;
  lastSdFullCheckMs = now;

  uint64_t totalBytes = SD.totalBytes();
  uint64_t usedBytes  = SD.usedBytes();

  if (totalBytes == 0) return; // can't check

  float usedPct = (float)usedBytes / (float)totalBytes * 100.0f;

  if (usedPct > SD_FULL_THRESHOLD_PCT && !sdNearlyFull) {
    sdNearlyFull = true;
  }

  if (sdNearlyFull && !sdFullAlertSent) {
    sdFullAlertSent = true;
    ...
    if (loraReady) {
      char msg[64];
      snprintf(msg, sizeof(msg), "ALERT:SD_FULL,%s", ts);
      lora.sendMessage(msg);
      loraCount++;
    }

    strncpy(lastAlertType, "SD_FULL", sizeof(lastAlertType) - 1);
    strncpy(lastAlertTs, ts, sizeof(lastAlertTs) - 1);

    Serial.printf("[ SD   ] ⚠ SD nearly full! %.1f%% used (%llu / %llu bytes)\n",
                  usedPct, usedBytes, totalBytes);
    Serial.printf("[ SD   ] LoRa alert sent: ALERT:SD_FULL,%s\n", ts);

    // Also log to SD
    File f = SD.open(LOG_FILE, FILE_APPEND);
    if (f) {
      f.printf("#ALERT:SD_FULL,%s\n", ts);
      f.close();
    }
  }
}
...
.panel{display:none;padding:26px 28px 80px;animation:fadeIn 0.25s ease}
...
/* ── Radar map (PortaPack-style) ─────────────────── */
.radar-wrap{position:relative;border-radius:var(--r);overflow:hidden;
  border:2px solid rgba(100,210,255,0.22);margin-bottom:16px;background:#08080c}
.radar-wrap::before{content:'';position:absolute;inset:0;z-index:400;pointer-events:none;
  background:radial-gradient(ellipse at center,transparent 45%,rgba(0,0,0,0.55) 100%)}
.radar-scanlines{position:absolute;inset:0;z-index:401;pointer-events:none;
  background:repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(100,210,255,0.012) 3px,rgba(100,210,255,0.012) 4px)}
.radar-hud-top{position:absolute;top:10px;left:50%;transform:translateX(-50%);z-index:402;
  padding:4px 14px;background:rgba(0,0,0,0.72);border:1px solid rgba(100,210,255,0.28);
  border-radius:4px;font-family:'SF Mono','Fira Code','Cascadia Code',monospace;
  font-size:10px;letter-spacing:2px;color:var(--cyan);white-space:nowrap}
.radar-hud-bottom{position:absolute;bottom:10px;left:10px;right:10px;z-index:402;
  display:flex;gap:8px;flex-wrap:wrap}
.hud-coord{padding:5px 10px;background:rgba(0,0,0,0.75);border:1px solid rgba(100,210,255,0.18);
  border-radius:4px;font-family:'SF Mono','Fira Code','Cascadia Code',monospace}
.hud-key{display:block;font-size:8px;color:var(--cyan);letter-spacing:1.5px;opacity:.65;text-transform:uppercase}
.hud-val{font-size:12px;color:var(--text);font-weight:500;font-variant-numeric:tabular-nums}
.radar-compass{position:absolute;top:10px;right:10px;z-index:402;width:30px;height:30px;
  display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.72);
  border:1px solid rgba(100,210,255,0.28);border-radius:50%;
  font-family:'SF Mono',monospace;font-size:11px;font-weight:700;color:var(--red)}
.radar-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
.radar-stat{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:14px 16px}
.radar-stat-lbl{font-size:9px;color:var(--cyan);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:6px}
.radar-stat-val{font-size:17px;font-weight:500;letter-spacing:-.5px;font-variant-numeric:tabular-nums}
.marker-pulse{position:relative;width:40px;height:40px}
.marker-dot{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  width:12px;height:12px;background:var(--cyan);border-radius:50%;
  box-shadow:0 0 8px var(--cyan),0 0 20px rgba(100,210,255,0.35)}
.marker-ring{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  width:12px;height:12px;border:2px solid var(--cyan);border-radius:50%;
  animation:radarPing 2s ease-out infinite}
@keyframes radarPing{0%{width:12px;height:12px;opacity:.7}100%{width:50px;height:50px;opacity:0}}
...
.alert-badge.fall{background:var(--yellow-soft);color:var(--yellow)}
.alert-badge.impact{background:var(--red-soft);color:var(--red)}
.alert-badge.sd-full{background:var(--orange-soft);color:var(--orange)}
.alert-time{color:var(--text-dim);margin-left:auto;font-size:12px}

/* ── Accelerometer panel ────────────────────────── */
.accel-hero{
  background:linear-gradient(135deg,rgba(100,210,255,0.10) 0%,rgba(10,132,255,0.06) 50%,transparent 100%);
  border:1px solid rgba(100,210,255,0.20);border-radius:var(--r);
  padding:32px 36px;display:flex;align-items:center;justify-content:space-between;
  gap:20px;margin-bottom:22px;position:relative;overflow:hidden}
.accel-hero::after{content:'📉';position:absolute;right:-10px;bottom:-20px;
  font-size:140px;opacity:0.04;pointer-events:none}
.accel-hero-lbl{font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;
  color:var(--cyan);margin-bottom:8px}
.accel-hero-val{font-size:68px;font-weight:300;letter-spacing:-4px;line-height:1}
.accel-hero-val span{font-size:26px;font-weight:400;color:var(--text-mid);letter-spacing:0;margin-left:3px}
.accel-hero-sub{font-size:18px;color:var(--text-dim);margin-top:6px;font-weight:400}
.accel-hero-meta{text-align:right}
.accel-hero-row{font-size:13px;color:var(--text-dim);margin-bottom:5px}
.accel-hero-row strong{color:var(--text-mid);font-weight:500}

.axis-card{position:relative}
.axis-bar-wrap{margin-top:14px;height:10px;background:var(--surface2);border-radius:5px;overflow:hidden}
.axis-bar-fill{height:100%;border-radius:5px;transition:width .25s ease}
.axis-bar-x{background:linear-gradient(90deg,var(--blue),var(--cyan))}
.axis-bar-y{background:linear-gradient(90deg,var(--green),var(--cyan))}
.axis-bar-z{background:linear-gradient(90deg,var(--orange),var(--yellow))}
.axis-dir{position:absolute;top:0;right:22px;display:flex;gap:4px;margin-top:6px}
.axis-dir span{font-size:10px;padding:2px 6px;border-radius:4px;
  background:var(--surface2);color:var(--text-dim);font-weight:500}

.gauge-ring{
  width:140px;height:140px;border-radius:50%;
  border:6px solid var(--surface2);position:relative;
  display:flex;align-items:center;justify-content:center;
  margin:0 auto;transition:border-color .3s}
.gauge-ring.low{border-color:var(--green)}
.gauge-ring.mid{border-color:var(--yellow)}
.gauge-ring.high{border-color:var(--red)}
.gauge-inner{text-align:center}
.gauge-val{font-size:32px;font-weight:300;letter-spacing:-1.5px;line-height:1}
.gauge-lbl{font-size:10px;color:var(--text-dim);margin-top:4px;text-transform:uppercase;letter-spacing:1px}

.peak-badge{
  display:inline-flex;align-items:center;gap:6px;padding:4px 12px;
  background:var(--red-soft);border:1px solid rgba(255,69,58,0.30);
  ...
  <nav class="sidebar">
    <div class="sidebar-logo">
      ...
      <div>
        ...
        <div class="logo-sub">TX · GPS · SD · IMU · ACCEL</div>
      </div>
    </div>
    ...
    <div class="nav">
      ...
      <div class="nav-item" onclick="showPanel('accel')" id="nav-accel">
        <span class="nav-icon">📲</span>
        <span class="nav-label">Accelerometer</span>
      </div>
    ...
    <div class="sidebar-footer">
      ...
      <div class="sf-row"><span>IMU Alerts</span><span class="sf-val" id="sf-alerts">0</span></div>
    </div>
  </nav>
  ...
  <div class="content">
    ...
    <div class="panels">
      ...
      <div class="panel" id="panel-map">
        <div class="section-title">📡 GPS Radar — Live Position</div>
        <div class="radar-wrap">
          <div class="radar-scanlines"></div>
          <div id="map"></div>
          <div class="radar-hud-top">TX TRACKING ACTIVE</div>
          <div class="radar-hud-bottom">
            <div class="hud-coord"><span class="hud-key">Lat</span><span class="hud-val" id="map-lat">---</span></div>
            <div class="hud-coord"><span class="hud-key">Lon</span><span class="hud-val" id="map-lon">---</span></div>
            <div class="hud-coord"><span class="hud-key">Alt</span><span class="hud-val" id="map-alt">--- m</span></div>
            <div class="hud-coord"><span class="hud-key">Sats</span><span class="hud-val" id="map-sats-hud">---</span></div>
          </div>
          <div class="radar-compass">N</div>
        </div>
        <div class="radar-stats">
          <div class="radar-stat">
            <div class="radar-stat-lbl">Latitude</div>
            <div class="radar-stat-val" id="map-lat-stat">---</div>
          </div>
          <div class="radar-stat">
            <div class="radar-stat-lbl">Longitude</div>
            <div class="radar-stat-val" id="map-lon-stat">---</div>
          </div>
          <div class="radar-stat">
            <div class="radar-stat-lbl">Altitude</div>
            <div class="radar-stat-val" id="map-alt-stat">--- m</div>
          </div>
        </div>
        <div style="margin-top:12px;font-size:11px;color:var(--text-dim);
          padding:10px 14px;background:var(--surface);border-radius:10px;border:1px solid var(--border);
          font-family:'SF Mono','Fira Code','Cascadia Code',monospace;letter-spacing:.3px">
          ⚡ Dark radar tiles via CartoDB — requires internet on your device.
          Marker updates with every valid GPS fix · Distance rings: 100 m · 500 m · 1 km · 5 km
        </div>
      </div>
      ...
      <div class="panel" id="panel-lora">
        ...
        <div style="margin-top:22px">
          ...
          <div id="alerts-list">
            <div style="text-align:center;padding:24px;color:var(--text-dim);font-size:13px;background:var(--surface);border-radius:var(--r);border:1px solid var(--border)">
        ...
        <div style="margin-top:14px;font-size:12px;color:var(--text-dim);
          ...
          Alert format: ALERT:TYPE,YYYY-MM-DD,HH:MM:SS
        </div>
      </div>
      ...
      <div class="panel" id="panel-sd">
        ...
        <div class="grid">
          <div class="card c-green">
            ...
            <div class="card-sub" id="sd-space-sub">FAT32 · SPI bus</div>
          </div>
          <div class="card c-blue">
      ...
      <div class="panel" id="panel-accel">
        <div class="section-title">Accelerometer — BMI160 Live Data</div>

        <div class="accel-hero">
          <div>
            <div class="accel-hero-lbl">Current Magnitude</div>
            <div class="accel-hero-val" id="accel-mag-hero">---<span>g</span></div>
            <div class="accel-hero-sub" id="accel-dir-label">Awaiting IMU data…</div>
          </div>
          <div class="accel-hero-meta">
            <div class="accel-hero-row">📈 Peak Record</div>
            <div style="font-size:28px;font-weight:600;color:var(--red);margin-bottom:10px" id="accel-peak-inline">--- g</div>
            <div class="accel-hero-row">Logged events <strong id="accel-events-inline">0</strong></div>
            <div class="accel-hero-row">Threshold <strong>> 2.0 g</strong></div>
          </div>
        </div>

        <div class="grid">
          <div class="card c-blue axis-card">
            <div class="card-label"><span class="card-icon">↔</span> X Axis</div>
            <div class="card-value" id="accel-x-val">---<span class="unit">g</span></div>
            <div class="axis-bar-wrap">
              <div class="axis-bar-fill axis-bar-x" id="accel-x-bar" style="width:50%"></div>
            </div>
            <div class="axis-dir">
              <span id="accel-x-dir" style="color:var(--cyan)">Lateral</span>
            </div>
          </div>

          <div class="card c-green axis-card">
            <div class="card-label"><span class="card-icon">↕</span> Y Axis</div>
            <div class="card-value" id="accel-y-val">---<span class="unit">g</span></div>
            <div class="axis-bar-wrap">
              <div class="axis-bar-fill axis-bar-y" id="accel-y-bar" style="width:50%"></div>
            </div>
            <div class="axis-dir">
              <span id="accel-y-dir" style="color:var(--green)">Forward</span>
            </div>
          </div>

          <div class="card c-orange axis-card">
            <div class="card-label"><span class="card-icon">⬆</span> Z Axis</div>
            <div class="card-value" id="accel-z-val">---<span class="unit">g</span></div>
            <div class="axis-bar-wrap">
              <div class="axis-bar-fill axis-bar-z" id="accel-z-bar" style="width:50%"></div>
            </div>
            <div class="axis-dir">
              <span id="accel-z-dir" style="color:var(--orange)">Vertical</span>
            </div>
          </div>

          <div class="card c-red">
            <div class="card-label"><span class="card-icon">🏆</span> Peak Acceleration</div>
            <div class="gauge-ring" id="peak-gauge">
              <div class="gauge-inner">
                <div class="gauge-val" id="accel-peak-val">---</div>
                <div class="gauge-lbl">Peak (g)</div>
              </div>
            </div>
            <div class="card-sub" style="text-align:center;margin-top:12px">
              Recorded at: <strong id="accel-peak-ts">---</strong>
            </div>
            <div style="text-align:center;margin-top:10px">
              <span class="peak-badge" id="accel-peak-badge" style="display:none">
                🔴 IMPACT LEVEL
              </span>
            </div>
          </div>
        ...
        <div class="data-top" style="margin-top:26px">
          <div class="section-title" style="margin-bottom:0;flex:1">Big Acceleration Events</div>
          <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
            <span class="data-count"><strong id="accelLogCountLabel">0</strong> events (>&nbsp;2.0&nbsp;g)</span>
            <button class="btn btn-ghost" onclick="refreshAccelLog()">↻ Refresh</button>
            <a href="/download" download="snowlog.csv">
              <button class="btn btn-primary">⬇ Download CSV</button>
            </a>
          </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Magnitude (g)</th><th>X (g)</th><th>Y (g)</th><th>Z (g)</th>
              </tr>
            </thead>
            <tbody id="accelLogBody">
              <tr><td colspan="6" style="text-align:center;padding:30px;color:var(--text-dim)">
                No acceleration events yet — threshold is 2.0 g
              </td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div style="margin-top:14px;font-size:12px;color:var(--text-dim);
          padding:12px 16px;background:var(--surface);border-radius:10px;border:1px solid var(--border)">
          📲 Events with magnitude > <strong>2.0 g</strong> are logged to <code>/accellog.csv</code> on the SD card.
          Impact alerts (> 3.5 g) also trigger LoRa notifications. Peak value updates in real-time.
        </div>

        <!-- Fall Detection Section -->
        <div style="margin-top:24px">
          <div class="section-title">🪂 Fall Detection Events</div>
          <div class="grid" style="margin-bottom:16px">
            <div class="card c-yellow">
              <div class="card-label"><span class="card-icon">🪂</span> Falls Detected</div>
              <div class="card-value" id="fall-count-val">0</div>
              <div class="card-sub">Free-fall events (&lt; 0.3 g for 150 ms)</div>
            </div>
            <div class="card c-red">
              <div class="card-label"><span class="card-icon">💥</span> Impacts</div>
              <div class="card-value" id="impact-count-val">0</div>
              <div class="card-sub">High-force events (&gt; 3.5 g)</div>
            </div>
          </div>

          <div class="data-top">
            <span class="data-count"><strong id="fallLogCountLabel">0</strong> fall events logged</span>
            <div style="display:flex;gap:10px;align-items:center">
              <button class="btn btn-ghost" onclick="refreshFallLog()">↻ Refresh</button>
            </div>
          </div>
          ...
          <div class="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>#</th><th>Timestamp</th>
                  <th>Magnitude (g)</th><th>Duration (ms)</th>
                  <th>X (g)</th><th>Y (g)</th><th>Z (g)</th>
                </tr>
              </thead>
              <tbody id="fallLogBody">
                <tr><td colspan="7" style="text-align:center;padding:30px;color:var(--text-dim)">
                  No fall events detected — station is stable
                </td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div style="margin-top:12px;font-size:11px;color:var(--text-dim);
            padding:10px 14px;background:var(--surface);border-radius:10px;border:1px solid var(--border)">
            🪂 Fall events are logged to <code>/falllog.csv</code> on the SD card with magnitude, duration, and axis data.
          </div>
        </div>
      </div>
...
const panels = {
  ...
  lora: {el:'panel-lora', nav:'nav-lora', title:'LoRa TX',          sub:'Transmission stats, alerts and last payload'},
  sd:   {el:'panel-sd',   nav:'nav-sd',   title:'SD Logger',        sub:'Log file status and session history'},
  accel:{el:'panel-accel',nav:'nav-accel',title:'Accelerometer',     sub:'BMI160 live data, peak & event log'},
...
function showPanel(id) {
  ...
  if (id === 'accel') { refreshAccelLog(); refreshFallLog(); }
...
function renderDash(d) {
  ...
  const bmiOk   = d.bmi160_ready || false;
  const totalAlerts = (d.fall_count||0) + (d.impact_count||0);
  ...
  document.getElementById('sf-alerts').textContent  = totalAlerts;
  ...
  document.getElementById('dash-inner').innerHTML = `
    ...
    <div class="grid">
      ...
      <div class="card ${bmiOk ? 'c-green' : 'c-red'}">
        <div class="card-label"><span class="card-icon">📲</span> BMI160 IMU</div>
        <div class="card-value" style="font-size:20px;letter-spacing:-.3px">${bmiOk ? 'ONLINE' : 'OFFLINE'}</div>
        <div class="card-sub">Falls: <strong>${d.fall_count||0}</strong> · Impacts: <strong>${d.impact_count||0}</strong></div>
        ${d.last_alert && d.last_alert !== '---' ? '<div class="card-sub">Last: ' + d.last_alert + '</div>' : ''}
      </div>

      ${d.sd_nearly_full ? `
      <div class="card c-red">
        <div class="card-label"><span class="card-icon">⚠️</span> SD Card</div>
        <div class="card-value" style="font-size:20px;letter-spacing:-.3px;color:var(--red)">NEARLY FULL</div>
        <div class="card-sub">Space usage exceeds 90%. Replace or clear the SD card.</div>
      </div>` : ''}
    </div>
...
function renderLora(d) {
  ...
  // Render alerts
  const totalAlerts = (d.fall_count||0) + (d.impact_count||0) + (d.sd_nearly_full ? 1 : 0);
  const alertsEl = el('alerts-list');
  if (!alertsEl) return;

  let html = '';
  if (totalAlerts === 0) {
    html = '<div style="text-align:center;padding:24px;color:var(--text-dim);font-size:13px;background:var(--surface);border-radius:var(--r);border:1px solid var(--border)">No alerts recorded yet</div>';
  } else {
    if (d.fall_count > 0) {
      html += `<div class="alert-item"><span class="alert-badge fall">⚠ Fall</span><span>Free-fall detected</span><span class="alert-time">${d.fall_count}x</span></div>`;
    }
    if (d.impact_count > 0) {
      html += `<div class="alert-item"><span class="alert-badge impact">💥 Impact</span><span>Impact detected</span><span class="alert-time">${d.impact_count}x</span></div>`;
    }
    if (d.sd_nearly_full) {
      html += `<div class="alert-item"><span class="alert-badge sd-full">💾 SD Full</span><span>SD card nearly full (>90%)</span><span class="alert-time">Active</span></div>`;
    }
  }
...
function renderSD(d) {
  ...
  if (d.sd_nearly_full && el('sd-space-sub')) {
    el('sd-space-sub').textContent = '⚠ SD nearly full! Replace or clear.';
    el('sd-space-sub').style.color = 'var(--red)';
  }
}
...
function renderAccel(d) {
  ...
  const mag = d.accel_mag || 0;

  // Hero magnitude
  if (el('accel-mag-hero')) {
    el('accel-mag-hero').innerHTML = bmiOk ? `${mag.toFixed(3)}<span>g</span>` : `---<span>g</span>`;
    el('accel-mag-hero').style.color = mag > 3.5 ? 'var(--red)' : mag > 2.0 ? 'var(--yellow)' : 'var(--cyan)';
  }

  // Direction label
  if (el('accel-dir-label')) {
    if (!bmiOk) {
      el('accel-dir-label').textContent = 'IMU offline';
    } else if (mag < 0.3) {
      el('accel-dir-label').textContent = '⚠ Free fall detected!';
      el('accel-dir-label').style.color = 'var(--yellow)';
    } else if (mag > 3.5) {
      el('accel-dir-label').textContent = '💥 Impact level!';
      el('accel-dir-label').style.color = 'var(--red)';
    } else if (mag > 2.0) {
      el('accel-dir-label').textContent = '⚡ High acceleration';
      el('accel-dir-label').style.color = 'var(--orange)';
    } else {
      el('accel-dir-label').textContent = 'Normal range';
      el('accel-dir-label').style.color = 'var(--text-dim)';
    }
  }

  // Peak inline
  if (el('accel-peak-inline')) el('accel-peak-inline').textContent = (d.peak_mag || 0).toFixed(3) + ' g';
  if (el('accel-events-inline')) el('accel-events-inline').textContent = d.accel_log_count || 0;

  // Axis cards — bars (range -2g to +2g → 0% to 100%)
  const gToPct = v => Math.min(100, Math.max(0, ((v + 2) / 4) * 100));
  if (el('accel-x-val')) el('accel-x-val').innerHTML = bmiOk ? `${ax.toFixed(3)}<span class="unit">g</span>` : `---<span class="unit">g</span>`;
  if (el('accel-y-val')) el('accel-y-val').innerHTML = bmiOk ? `${ay.toFixed(3)}<span class="unit">g</span>` : `---<span class="unit">g</span>`;
  if (el('accel-z-val')) el('accel-z-val').innerHTML = bmiOk ? `${az.toFixed(3)}<span class="unit">g</span>` : `---<span class="unit">g</span>`;
  if (el('accel-x-bar')) el('accel-x-bar').style.width = gToPct(ax) + '%';
  if (el('accel-y-bar')) el('accel-y-bar').style.width = gToPct(ay) + '%';
  if (el('accel-z-bar')) el('accel-z-bar').style.width = gToPct(az) + '%';

  // Axis direction labels
  if (el('accel-x-dir')) el('accel-x-dir').textContent = ax >= 0 ? 'Right +' : 'Left −';
  if (el('accel-y-dir')) el('accel-y-dir').textContent = ay >= 0 ? 'Forward +' : 'Backward −';
  if (el('accel-z-dir')) el('accel-z-dir').textContent = az >= 0 ? 'Up +' : 'Down −';

  // Peak gauge
  const peakM = d.peak_mag || 0;
  if (el('accel-peak-val')) el('accel-peak-val').textContent = peakM.toFixed(3);
  if (el('accel-peak-ts')) el('accel-peak-ts').textContent = d.peak_ts || '---';
  if (el('peak-gauge')) {
    el('peak-gauge').className = 'gauge-ring ' + (peakM > 3.5 ? 'high' : peakM > 2.0 ? 'mid' : 'low');
  }
  if (el('accel-peak-badge')) {
    el('accel-peak-badge').style.display = peakM > 3.5 ? 'inline-flex' : 'none';
  }
  // Fall/Impact count cards
  if (el('fall-count-val')) el('fall-count-val').textContent = d.fall_count || 0;
  if (el('impact-count-val')) el('impact-count-val').textContent = d.impact_count || 0;
}

async function refreshAccelLog() {
  try {
    const r = await fetch('/accellog');
    const data = await r.json();
    renderAccelLog(data);
  } catch(e) {}
}

function renderAccelLog(data) {
  const el = id => document.getElementById(id);
  el('accelLogCountLabel').textContent = data.count || 0;

  if (!data.entries || !data.entries.length) {
    el('accelLogBody').innerHTML =
      '<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--text-dim)">' +
      'No acceleration events yet — threshold is ' + (data.threshold||2.0) + ' g</td></tr>';
    return;
  }

  const rows = data.entries.map((e, i) => {
    const isImpact = e.mag > 3.5;
    const rowClass = isImpact ? 'td-bad' : e.mag > 2.5 ? 'td-warn' : 'td-good';
    return '<tr>' +
      '<td>' + (i + 1) + '</td>' +
      '<td>' + e.ts + '</td>' +
      '<td class="' + rowClass + '">' + e.mag.toFixed(3) + ' g' + (isImpact ? ' 💥' : '') + '</td>' +
      '<td>' + e.ax.toFixed(3) + '</td>' +
      '<td>' + e.ay.toFixed(3) + '</td>' +
      '<td>' + e.az.toFixed(3) + '</td>' +
      '</tr>';
  }).reverse().join('');
  el('accelLogBody').innerHTML = rows;
}

// ══ FALL LOG ══════════════════════════════════════
async function refreshFallLog() {
  try {
    const r = await fetch('/falllog');
    const data = await r.json();
    renderFallLog(data);
  } catch(e) {}
}

function renderFallLog(data) {
  document.getElementById('fallLogCountLabel').textContent = data.count || 0;
  if (!data.entries || !data.entries.length) {
    document.getElementById('fallLogBody').innerHTML =
      '<tr><td colspan="7" style="text-align:center;padding:30px;color:var(--text-dim)">' +
      'No fall events detected — station is stable</td></tr>';
    return;
  }
  const rows = data.entries.map((e, i) =>
    '<tr><td>' + (i+1) + '</td><td>' + e.ts + '</td>' +
    '<td class="td-warn">' + e.mag.toFixed(3) + ' g</td>' +
    '<td>' + e.dur + ' ms</td>' +
    '<td>' + e.ax.toFixed(3) + '</td><td>' + e.ay.toFixed(3) + '</td><td>' + e.az.toFixed(3) + '</td></tr>'
  ).reverse().join('');
  document.getElementById('fallLogBody').innerHTML = rows;
}
...
async function fetchDash() {
  try {
    ...
    document.getElementById('sf-txcount').textContent = d.lora_count;
    document.getElementById('sf-sdrows').textContent  = d.sd_ready ? d.sd_rows : 'No SD';
    document.getElementById('sf-alerts').textContent  = (d.fall_count||0) + (d.impact_count||0);
    // Render active panel
    if (activePanel === 'dash') renderDash(d);
    if (activePanel === 'lora') renderLora(d);
    if (activePanel === 'sd')   renderSD(d);
    if (activePanel === 'accel') renderAccel(d);
...
async function initOrUpdateMap() {
  ...
  if (!d || !d.gps_valid || +d.lat === 0) {
    document.getElementById('map').innerHTML =
      ...
      '<div style="font-size:40px;opacity:.3">📡</div>No GPS fix available yet</div>';
    mapInitialized = false;
    return;
  }
  const lat = +d.lat, lon = +d.lon;

  // Update all HUD readouts
  document.getElementById('map-lat').textContent = lat.toFixed(5);
  document.getElementById('map-lon').textContent = lon.toFixed(5);
  document.getElementById('map-alt').textContent = (+d.alt).toFixed(1) + ' m';
  document.getElementById('map-sats-hud').textContent = (d.sats || 0) + ' sat';
  document.getElementById('map-lat-stat').textContent = lat.toFixed(5) + ' °N';
  document.getElementById('map-lon-stat').textContent = lon.toFixed(5) + ' °E';
  document.getElementById('map-alt-stat').textContent = (+d.alt).toFixed(1) + ' m';

  if (!mapInitialized) {
    leafletMap = L.map('map', {zoomControl: false}).setView([lat, lon], 15);
    L.control.zoom({position: 'bottomright'}).addTo(leafletMap);

    // Dark radar tiles (CartoDB dark matter)
    ...
    const pulseIcon = L.divIcon({
      html: '<div class="marker-pulse"><div class="marker-dot"></div><div class="marker-ring"></div></div>',
      iconSize: [40, 40], iconAnchor: [20, 20], className: ''
    });
    mapMarker = L.marker([lat, lon], { icon: pulseIcon }).addTo(leafletMap);
    mapMarker.bindPopup(
      '<b style="color:#64d2ff">❄ Snow Transmitter</b><br>' +
      'Depth : ' + (d.depth >= 0 ? d.depth.toFixed(1) + ' cm' : 'N/A') + '<br>' +
      'Alt   : ' + (+d.alt).toFixed(0) + ' m<br>' +
      (d.date||'') + ' ' + (d.time_s||'')
    ).openPopup();

    // Distance rings (station doesn't move — create once)
    [100, 500, 1000, 5000].forEach(m => {
      L.circle([lat, lon], {
        radius: m, color: 'rgba(100,210,255,0.12)', fillColor: 'transparent',
...
void handleData() {
  ...
  snprintf(json, sizeof(json),
    ...
    "\"last_alert\":\"%s\","
    "\"sd_nearly_full\":%s,"
    "\"accel_x\":%.3f,\"accel_y\":%.3f,\"accel_z\":%.3f,\"accel_mag\":%.3f,"
    "\"peak_mag\":%.3f,\"peak_x\":%.3f,\"peak_y\":%.3f,\"peak_z\":%.3f,\"peak_ts\":\"%s\","
    "\"accel_log_count\":%d,"
    "\"fall_log_count\":%d"
    "}",
    distCm,
    ...
    readAge,
    safePayload,
    bmi160Ready              ? "true"  : "false",
    fallCount,
    impactCount,
    lastAlertTs,
    sdNearlyFull             ? "true"  : "false",
    liveAccelX, liveAccelY, liveAccelZ, liveAccelMag,
    peakMag, peakX, peakY, peakZ, peakTs,
    accelLogCount,
    fallLogCount
  );
...
void handleAccelLog() {
  ...
  json += accelLogCount;
  json += ",\"threshold\":";
  json += ACCEL_LOG_THRESHOLD_G;
  json += ",\"peak\":";
  char peakBuf[120];
  snprintf(peakBuf, sizeof(peakBuf),
    "{\"mag\":%.3f,\"x\":%.3f,\"y\":%.3f,\"z\":%.3f,\"ts\":\"%s\"}",
    peakMag, peakX, peakY, peakZ, peakTs);
  json += peakBuf;
  json += ",\"entries\":[";

  int start = (accelLogCount < ACCEL_LOG_MAX_ENTRIES) ? 0 : accelLogHead;
  for (int i = 0; i < accelLogCount; i++) {
    int idx = (start + i) % ACCEL_LOG_MAX_ENTRIES;
    AccelEvent& e = accelLog[idx];
    if (i) json += ',';
    char entry[120];
    snprintf(entry, sizeof(entry),
      "{\"ts\":\"%s\",\"mag\":%.3f,\"ax\":%.3f,\"ay\":%.3f,\"az\":%.3f}",
      e.ts, e.mag, e.ax, e.ay, e.az);
    json += entry;
  }
  json += "]}";
...
void handleFallLog() {
  ...
  int start = (fallLogCount < FALL_LOG_MAX_ENTRIES) ? 0 : fallLogHead;
  for (int i = 0; i < fallLogCount; i++) {
    int idx = (start + i) % FALL_LOG_MAX_ENTRIES;
    FallEvent& e = fallLog[idx];
    if (i) json += ',';
    char entry[160];
    snprintf(entry, sizeof(entry),
      "{\"ts\":\"%s\",\"mag\":%.3f,\"dur\":%lu,\"ax\":%.3f,\"ay\":%.3f,\"az\":%.3f}",
      e.ts, e.mag, (unsigned long)e.durationMs, e.ax, e.ay, e.az);
    json += entry;
  }
  json += "]}";
...
void writeAccelLogHeader() {
  if (SD.exists(ACCEL_LOG_FILE)) return;
  File f = SD.open(ACCEL_LOG_FILE, FILE_WRITE);
  if (!f) return;
  f.println("timestamp,magnitude_g,x_g,y_g,z_g");
  f.close();
  Serial.println("[ SD   ] Header written to " ACCEL_LOG_FILE);
}

void writeFallLogHeader() {
  if (SD.exists(FALL_LOG_FILE)) return;
  File f = SD.open(FALL_LOG_FILE, FILE_WRITE);
  if (!f) return;
  f.println("timestamp,magnitude_g,duration_ms,x_g,y_g,z_g");
  f.close();
  Serial.println("[ SD   ] Header written to " FALL_LOG_FILE);
}
...
void printReport() {
  ...
  Serial.print("[ IMU   ] BMI160      : ");
  if (bmi160Ready)
    Serial.printf("ONLINE — falls: %lu  impacts: %lu\n", fallCount, impactCount);
  else
    Serial.println("OFFLINE");

  if (sdNearlyFull)
    Serial.println("[ SD   ] ⚠ SD card nearly full!");

  if (strcmp(lastAlertTs, "---") != 0)
    Serial.printf("[ IMU   ] Last alert   : %s (%s)\n", lastAlertTs, lastAlertType);
...
void logToSD() {
  ...
  // Check SD full after each write
  checkSdFull();
}
...
void setup() {
  ...
  // ── BMI160 IMU (I2C) ────────────────────────────
  Wire.begin(BMI160_SDA_PIN, BMI160_SCL_PIN);
  Serial.printf("[ IMU  ] Initialising BMI160 on I2C (SDA=%d, SCL=%d, addr=0x%02X)…\n",
                BMI160_SDA_PIN, BMI160_SCL_PIN, BMI160_I2C_ADDR);
  if (BMI160.begin(BMI160GenClass::I2C_MODE, BMI160_I2C_ADDR)) {
    bmi160Ready = true;
    BMI160.setAccelerometerRange(BMI160_ACCEL_RANGE_2G);
    Serial.println("[ IMU  ] BMI160 initialised OK — range ±2g");
    Serial.printf("[ IMU  ] Fall threshold : < %.1f g  |  Impact threshold : > %.1f g\n",
                  BMI160_FALL_THRESHOLD_G, BMI160_IMPACT_THRESHOLD_G);
    Serial.printf("[ IMU  ] Alert cooldown : %d ms\n", BMI160_ALERT_COOLDOWN_MS);
  } else {
    bmi160Ready = false;
    Serial.println("[ IMU  ] BMI160 init FAILED — check I2C wiring (SDA=21, SCL=22)");
  }
  ...
  server.on("/accellog", HTTP_GET, handleAccelLog);
  server.on("/falllog",  HTTP_GET, handleFallLog);
  ...
  Serial.println();
  Serial.println("═══════════════════════════════════════════════");
  Serial.println("  Snow Transmitter v4 — ready");
  Serial.printf("  WiFi : %s / %s\n", WIFI_AP_SSID, WIFI_AP_PASS);
  Serial.printf("  URL  : http://%s\n", WiFi.softAPIP().toString().c_str());
  Serial.printf("  IMU  : BMI160 %s\n", bmi160Ready ? "ONLINE" : "OFFLINE");
  Serial.println("═══════════════════════════════════════════════");
  Serial.println();
}
...
void loop() {
  ...
  // IMU check on every loop iteration for responsiveness
  checkBMI160();
